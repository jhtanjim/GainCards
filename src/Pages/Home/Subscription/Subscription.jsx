...
import { Check, Crown, Diamond, Heart, Rocket, Star, Zap } from "lucide-react";
...
const Subscription = () => {
  ...
  const calculateDiscountedPrice = (originalPrice, discountPct) => {
    ...
    return Math.round(originalPrice - (originalPrice * discountPct) / 100);
  };
  ...
  const getEnhancedPlans = (apiPlans) => {
    ...
    return apiPlans.map((plan, index) => {
      ...
      const finalPrice = plan.discountPct > 0 ? discountedPrice : originalPrice;
      ...
      <div className="relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg sm:text-xl text-white/70 max-w-2xl mx-auto leading-relaxed px-4">
            Flexible plans for every type of collector — whether you're just
            starting or a seasoned vendor.
          </p>
        </div>
            ...
            {enhancedPlans.map((plan, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 3D Card Glow */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${plan.bgGradient} rounded-3xl blur-xl group-hover:blur-2xl transition-all duration-700 opacity-50 group-hover:opacity-80`}
                  ></div>
                  ...
                  <div className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 sm:p-8 hover:bg-white/10 transition-all duration-700 hover:scale-[1.02] hover:border-white/20 overflow-hidden h-full">
                    ...
                    <div className="absolute top-4 right-4 sm:top-6 sm:right-6 opacity-10 group-hover:opacity-30 transition-opacity duration-700">
                      <Icon
                        size={60}
                        className={`text-${plan.color}-400 sm:w-20 sm:h-20`}
                      />
                    </div>
                    <div className="absolute -bottom-4 -left-4 sm:-bottom-6 sm:-left-6 opacity-5 group-hover:opacity-20 transition-opacity duration-700">
                      <Zap
                        size={80}
                        className={`text-${plan.color}-300 sm:w-30 sm:h-30`}
                      />
                    </div>
                    ...
                    <div className="relative z-10 h-full flex flex-col">
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
                        <div
                          className={`w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br ${plan.gradient} rounded-2xl flex items-center justify-center group-hover:rotate-12 transition-transform duration-700 shadow-2xl`}
                        >
                          <Icon
                            size={20}
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 capitalize truncate">
                            {plan.name}
                          </h3>
                          <div
                            className={`w-6 sm:w-8 h-1 bg-gradient-to-r ${plan.gradient} rounded-full group-hover:w-16 sm:group-hover:w-20 transition-all duration-700`}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div className="mb-6 sm:mb-8">
                        ...
                        {plan.discountPct > 0 && (
                          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                            ...
                            <span
                              className={`bg-gradient-to-r ${plan.gradient} text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold shadow-lg`}
                            >
                              {plan.discountPct}% OFF
                            </span>
                            <span className="text-green-400 text-xs sm:text-sm font-medium">
                              Save ${plan.originalPrice - plan.finalPrice}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-grow">
                        {plan.features.map((feature, idx) => (
                          ...
                          >
                            <div
                              className={`w-5 h-5 sm:w-6 sm:h-6 bg-gradient-to-r ${plan.gradient} rounded-full flex items-center justify-center flex-shrink-0`}
                            >
                              <Check
                                size={12}
                      ...
                      {/* CTA Button */}
                      {!isVendor && !isAdmin && (
                        <Link to="/vendorSignup" className="block">
                          <button
                            className={`group/btn relative w-full px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r ${plan.gradient} rounded-2xl text-white font-semibold text-base sm:text-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 overflow-hidden`}
                          >
                            ...
                            <div
                              className={`absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500`}
                            ></div>
                          </button>
                        </Link>
                      )}

                      {/* Show current plan status for vendors/admins */}
                      {(isVendor || isAdmin) && (
                        <div className="text-center">
                          <span className="text-white/60 text-sm">
                            {isCurrent ? "Your Current Plan" : "Available Plan"}
                          </span>
                        </div>
                      )}
                    </div>
